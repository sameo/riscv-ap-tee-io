...
TDIs PCIe Device Capabilities Registers must have the TEE-IO bit set in order
for the host to discover their TDISP capability.
...
TODO: List of required SPDM messages
...
consequence, CoVE-IO compatible devices must implement and expose this PCIe
capability.
...
The CoVE-IO compatible devices must keep tracking the IDE key invocation field,
which is the bit 63:0 of the IV. If the IDE key invocation field overflow
happens, the IDE stream must enter Insecure. Before overflow, the device may
signal event to the host to let the host software perform key refresh process
with IDE_KM protocol.
